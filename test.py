...
def initialize_database():
    ...
    # Создаем таблицу store, если она не существует
    cursor.execute('''
...
def show_store(message):
    ...
    # Инициализируем состояние пользователя, если оно ещё не существует
    if chat_id not in user_states:
        user_states[chat_id] = {}
    user_states[chat_id]['state'] = "choosing_product"
    ...
    # Регистрируем обработчики кнопок для каждого товара
    for product in products:
        product_id, name, price, stock = product
        button_text = f"{name} - {price} баллов"
        markup.add(types.KeyboardButton(button_text))

        # Устанавливаем состояние для этой кнопки
        user_states[chat_id][button_text] = product_id

    markup.add(types.KeyboardButton("Отмена"))

    bot.send_message(chat_id, "Доступные товары в магазине:", reply_markup=markup)
    bot.send_message(chat_id, "Для покупки выберите товар из списка или нажмите 'Отмена' для отмены и возвращения в главное меню.")

    conn.close()

# Обработчик ввода сообщений от пользователя
@bot.message_handler(func=lambda message: True)
def handle_messages(message):
    chat_id = message.chat.id

    if chat_id in user_states and user_states[chat_id]['state'] == "choosing_product":
        # Если пользователь находится в состоянии выбора товара, вызываем обработчик buy_product
        buy_product(message)
    else:
        # Иначе обрабатываем другие сообщения
        show_main_menu(chat_id)

# Обработчик для покупки товара
def buy_product(message):
    chat_id = message.chat.id
    product_id = user_states[chat_id][message.text]  # Получаем ID товара из состояния
    ...
    if product_data:
        product_name, product_price, product_stock = product_data
        ...
        if product_stock > 0:
            ...
            bot.send_message(chat_id, f"Вы успешно купили товар '{product_name}' за {product_price} баллов.")
        else:
            bot.send_message(chat_id, "К сожалению, товар закончился в наличии.")
    else:
